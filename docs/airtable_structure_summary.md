...
| realized_cost | rollup | Actual cost incurred in USD. Format: #####.## |
...
| record_audit | aiText | Evaluates the strategic planning quality of a phase based on specific criteria and outputs a score with detailed analysis. |
| record_score | number | Extracts the assigned score from the record_audit field, defaulting to 0 if no score is present. |
...
| attachments | multipleAttachments | Related files and documents. Upload supporting materials here. |
| source | multilineText | Source files from workspace that contributed content to this record |
| record_audit | aiText | Evaluates the completeness and uniqueness of content within a record, assigning a score based on quality. |
| record_score | number | Extracts the agent assigned score from the record audit text data. |
...
| source | multilineText | Source files from workspace that contributed content to this record |
| record_audit | aiText | Evaluates the implementation quality of a task record based on code and specific criteria. |
| record_score | number | Extracts the agent assigned score from the 'record_audit' cell content. |
| ABCX | singleSelect |  |
| reconciled | singleSelect | Indicates whether the task_id, plan_link, phase_link, and stage_link are reconciled. |
