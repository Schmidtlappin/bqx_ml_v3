# Bulletproof record_audit Prompt - Empty Description Fix

## Critical Issue

Tasks with empty descriptions can still score 95+ even though they should be capped at 55.

## Root Cause

The current prompt may not be enforcing the -30 penalty strongly enough, or the penalty application order is unclear.

## Solution: Ultra-Explicit Empty Description Check

Replace the Tasks.record_audit prompt with this version that makes empty description checking the FIRST and most critical step:

---

## COMPLETE BULLETPROOF PROMPT
...
3. **TASK_ID MISSING?** → Subtract 50 points (critical failure)
...
- Add task_id (5) + name (15) + notes (30) + source (5) + stage_link (5) + status (5) = +65 attempted
...
"CRITICAL: Description field is empty or <20 characters. This limits maximum score to 55 points. Add comprehensive description (>100 chars) describing WHAT this task does, including methods, metrics, and expected outcomes."
...
1. **Navigate to AirTable**:
   - BQX ML V3 Base → Tasks table
   - Click on `record_audit` field
   - Select "Configure AI" or "Edit Prompt"

2. **Replace entire prompt** with the bulletproof version above

3. **Save and wait** for AI to rescore (10-30 minutes)
...
5. **Verify** empty description tasks score ≤55

## Key Changes from Previous Version

| Aspect | Previous | Bulletproof |
|--------|----------|-------------|
| Empty check placement | In scoring section | **FIRST THING, before anything else** |
| Empty check emphasis | One mention | **Repeated 5+ times, ALL CAPS** |
| Calculation clarity | Implicit | **Step-by-step with examples** |
| Validation | End of prompt | **Built-in self-check at end** |
| Error handling | None | **Explicit cap at 55 if empty** |
| Output requirements | General | **Must explicitly state if empty** |

## Expected Results

**With empty description:**
- Base: 40 - 30 = 10
- Max from other fields: 45
- **Maximum possible: 55** ✓

**With good description:**
- Base: 40
- Max from all fields: 85
...
# Expected output:
#   Empty description task: Score = 55 or less
#   Good description task: Score = 90+
```
