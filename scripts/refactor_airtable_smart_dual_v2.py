...
Version 2: Corrected field names
"""
...
- Update AirTable for each model"""
    },
    {
        ...
        'name': 'Create Feature Importance Monitor',
        'status': 'todo',
        'priority': 'medium',
...
for task_data in new_tasks:
    ...
    exists = False
    for record in all_tasks:
        if record['fields'].get('task_id') == task_data['task_id']:
            exists = True
            break
    ...
    if not exists:
        try:
            ...
            print(f"✅ Created: {task_data['task_id']} - {task_data['name']}")
            refactored += 1
        except Exception as e:
...
for record in all_tasks:
    task = record['fields']
    if task.get('task_id') == 'MP03.P01.S01.T01':
